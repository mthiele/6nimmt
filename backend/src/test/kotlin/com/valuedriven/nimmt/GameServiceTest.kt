...
class GameServiceTest {
    ...
    fun `sets up game correctly for two players`() {
        ...
        verify(simpMessagingTemplate).convertAndSend(eq("/topic/games"),
        ...
        verify(simpMessagingTemplate).convertAndSendToUser(eq("user"), eq("/queue/activeGames/123456/roundState"),
    ...
    fun `play a game with 2 players`() {
        ...
        val roundStateField = GameService::class.java.getDeclaredField("roundState")
        roundStateField.isAccessible = true
        roundStateField.set(cut, mutableMapOf("123456" to roundState))
        ...
        verify(simpMessagingTemplate).convertAndSendToUser("user",
                "/queue/activeGames/123456",
                PlayedCardMessage(PlayedCardHidden("user")))
        verify(simpMessagingTemplate).convertAndSendToUser("a dude",
                "/queue/activeGames/123456",
                PlayedCardMessage(PlayedCardHidden("user")))
        cut.getRoundState(user, "123456")
        verify(simpMessagingTemplate).convertAndSendToUser(eq("user"),
                eq("/queue/activeGames/123456/roundState"),
                argThat(PrivateRoundStateMatcher(
                        PrivateRoundState(
                                stepNumber = 1,
                                numberOfPlayers = 2,
                                rows = listOf(),
                                playedCards = mapOf("user" to null),
                                playerState = PlayerState(
                                        heap = listOf(),
                                        deck = listOf(
                                                Card(5),
        ...
        verify(simpMessagingTemplate).convertAndSendToUser("user", "/queue/activeGames/123456",
        ...
        val expectedRows = UpdatedRowsMessage(listOf(
                Row(0, listOf(Card(30))),
                Row(1, listOf(Card(40))),
                Row(2, listOf(Card(4))),
                Row(3, listOf(Card(104)))
        ...
        val expectedRows2 = UpdatedRowsMessage(listOf(
                Row(0, listOf(Card(30))),
                Row(1, listOf(Card(40), Card(96))),
                Row(2, listOf(Card(4))),
                Row(3, listOf(Card(104)))
        ))
        verify(simpMessagingTemplate).convertAndSendToUser("user", "/queue/activeGames/123456", expectedRows2)
        verify(simpMessagingTemplate).convertAndSendToUser("a dude", "/queue/activeGames/123456", expectedRows2)
        verify(simpMessagingTemplate).convertAndSendToUser("user", "/queue/activeGames/123456",
                StartStepMessage(PrivateRoundState(
                        stepNumber = 2,
                        numberOfPlayers = 2,
        ...
        verify(simpMessagingTemplate).convertAndSendToUser("a dude", "/queue/activeGames/123456",
                StartStepMessage(PrivateRoundState(
                        stepNumber = 2,
                        numberOfPlayers = 2,
                        rows = expectedRows2.payload!!,
